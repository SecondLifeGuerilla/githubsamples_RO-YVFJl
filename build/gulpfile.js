					...
					execute: function () {
						...
						gulp.task('js', () => {
							...
							let tasks = folders.map(folder => {
								return gulp.src([`!samples/${folder}/build/*`, `samples/${folder}/**/*.js`]).pipe(plumber()).pipe(sourcemaps.init()).pipe(babel()).pipe(plumber.stop()).pipe(sourcemaps.write('.')).pipe(gulp.dest(`./samples/${folder}/build`));
							});
